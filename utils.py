...
def fetch_pool_keys(pair_address: str) -> dict:
    try:
        ...
        # Извлечение ликвидности из base_vault и quote_vault
        ...
        if base_account_info.value:
            raw_base_liquidity = int.from_bytes(base_account_info.value.data[64:72], "little")
            pool_keys["base_liquidity"] = raw_base_liquidity / 10**amm_data_decoded.coinDecimals
            print(f"Parsed Base Liquidity: {pool_keys['base_liquidity']:.2f} SOL")
        else:
            pool_keys["base_liquidity"] = 0

        if quote_account_info.value:
            raw_quote_liquidity = int.from_bytes(quote_account_info.value.data[64:72], "little")
            pool_keys["quote_liquidity"] = raw_quote_liquidity / 10**amm_data_decoded.pcDecimals
            print(f"Parsed Quote Liquidity: {pool_keys['quote_liquidity']:.2f} Tokens")
        else:
            pool_keys["quote_liquidity"] = 0

        # Получение текущей цены токена
        token_price, _ = get_token_price(pool_keys)
        if token_price is None:
            print("Не удалось получить цену токена.")
            return None

        # Определение минимальной ликвидности в SOL для котируемого токена
        min_quote_liquidity_sol = 500  # Пример: эквивалент 25 SOL
        quote_liquidity_sol = pool_keys["quote_liquidity"] * token_price
        print(f"Quote Liquidity in SOL: {quote_liquidity_sol:.2f} SOL")

        # Установка минимальной ликвидности для базового токена (например, SOL)
        min_base_liquidity = 500  # В SOL

        if (pool_keys["base_liquidity"] < min_base_liquidity or 
            quote_liquidity_sol < min_quote_liquidity_sol):
            print(f"Liquidity too low: Base = {pool_keys['base_liquidity']:.2f} SOL, Quote = {quote_liquidity_sol:.2f} SOL")
            return None

        return pool_keys
    except Exception as e:
        print(f"Error in fetch_pool_keys: {e}")
        return None
...
def confirm_txn(txn_sig: Signature, max_retries: int = 10, retry_interval: int = 1) -> bool:
    retries = 1
